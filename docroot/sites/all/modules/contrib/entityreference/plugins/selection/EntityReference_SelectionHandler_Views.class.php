...
class EntityReference_SelectionHandler_Views implements EntityReference_SelectionHandler {
  ...
  public static function settingsForm($field, $instance) {
    ...
    if ($options) {
      ...
      // Add Group parent Entity selection form.
      $form['view']['entity_ref_selection_override'] = array(
        '#type' => 'select',
        '#title' => t('Pass group parent id as argument'),
  ...
  public function getReferencableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
    ...
    elseif (variable_get('er-22-' . $this->field['field_name']) == 2 && !empty($_SESSION['og_context']['gid'])) {
      $arg_array = og_get_entity_groups('node', $_SESSION['og_context']['gid']);
      foreach ($arg_array['node'] as $key => $a_array) {
